# VoxCPM HTTP server (Deepulse TTS adapter 后端)

把 [VoxCPM2](https://github.com/OpenBMB/VoxCPM) 的 Python in-process API 包成
`http://127.0.0.1:8001/synthesize` 给 Deepulse 后端 (Node/TS) 调用.
...
VoxCPM 是 Python lib (load 模型, 调 `model.generate(...)` 拿 numpy wav). Deepulse
...
平常**不用手动起** — `deepulse.bat` 在 `TTS=voxcpm` 时会自动 `start` 一个新窗
跑 `python app.py`. 用户只管 `deepulse.bat` 双击.

bat 会先看 :8001 端口在不在跑 — 在就跳过, 不重复起.

想单独跑 (e.g. 不开 Deepulse 只想测 vox):
...
shutdown 时 Deepulse 那边 Ctrl+C 只关 PWA + server, 不会关 vox 窗 (它是独立
...
返回 `{ "status": "ok", "model": "openbmb/VoxCPM2" }`. 给 Deepulse 启动时探活用.
...
| 8001 已占                       | `set VOXCPM_PORT=8002` 再起; 同时 Deepulse 那边 VOXCPM_URL 改 |
